...
pub(super) fn claude_models(conta: Option<&crate::contas_claude::Alvo>) -> serde_json::Value {
    // Off the UI thread: waiting here gets the updated runner, not one half copied (1.6.60).
...
/// 1.6.60 this paragraph sat at the top of `login.rs`, left there by the 1.6.41 split.)
...
///
/// Any runner the app ships (1.6.60: the Codex one too). The caller holds the runner's install
...
mod tests_motor {
    ...
    fn a_fonte_do_runner_e_procurada_em_mais_de_um_lugar() {
        ...
        // 1.6.60: the Codex runner travels too, or the app could never update it.
