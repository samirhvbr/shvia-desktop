...
//!
//! Escrito para **objc2 0.6 / objc2-web-kit 0.3** (as versões que o wry 0.55
//! resolve — ver Cargo.lock). Sem comando/IPC Tauri (ADR-001): é o canal nativo
//! do próprio WebKit, como no Linux.
...
impl Handler {
    fn new(window: WebviewWindow) -> Retained<Self> {
        let this = Self::alloc().set_ivars(Ivars { window });
...
pub fn install(window: &WebviewWindow) {
    ...
    let _ = window.with_webview(move |wv| {
        ...
        let handler = Handler::new(win.clone());
